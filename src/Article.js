...
function Article() {
  ...
  function fetchData(searchValue) {
    if (searchValue) {
        fetch('https://en.wikipedia.org/w/api.php?action=opensearch&search=' + searchValue + '&limit=10&namespace=0&origin=*')
          .then(results => {
            return results.json();
          }).then(data => {
            let articles = convertToJSONQuery(data).map((article, i) => {
              return (
                <a className="article" rel="noopener noreferrer" key={i} target="_blank" href={article.link}>
                  <h3> {article.title} </h3>
    ...
    <div className="entry">
      <div className="searchContainer">
        <input
          ...
          name="query"
        />
        <button className="clear" onClick={function(){clearQuery(); setMagazine([])}}>  <i className="fa fa-times"> </i> </button>
      </div>
      <h3>- or -</h3>
